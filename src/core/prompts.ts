...
const TOOL_INSTRUCTIONS = `# System
...
- CRITICAL: Always create NEW commits rather than amending, unless the user explicitly requests a git amend. When a pre-commit hook fails, the commit did NOT happen — so --amend would modify the PREVIOUS commit, which may result in destroying work. Instead, after hook failure, fix the issue, re-stage, and create a NEW commit
...
3. Run the commands to stage files and create the commit. Use a HEREDOC to pass the commit message.
...
2. Analyze all changes that will be included in the pull request, making sure to look at ALL commits (not just the latest), and draft a PR title and summary:
...
const AGENT_TOOL_INSTRUCTIONS = `# Agent tool
...
export function getAgentToolInstructions(): Promise<string | null> {
  return Promise.resolve(AGENT_TOOL_INSTRUCTIONS);
}

const MEMORY_INSTRUCTIONS = `# Auto Memory

You have access to a persistent, file-based memory system that survives across conversations. Use it to store information that will be useful in future sessions.
...
- **user** — User's role, preferences, expertise, goals. Helps tailor your behavior.
- **feedback** — Corrections or confirmed approaches from the user. Lead with the rule, then **Why:** and **How to apply:** lines.
- **project** — Ongoing work, decisions, constraints not derivable from code/git. Lead with the fact, then **Why:** and **How to apply:** lines.
- **reference** — Pointers to external resources (URLs, tools, dashboards).

## When to Save

- When the user explicitly asks you to remember something — save immediately.
- When you learn user preferences, role, or expertise.
- When the user corrects your approach OR confirms a non-obvious approach worked.
- When you learn project context not derivable from code or git history.
- When you discover useful external resource locations.
...
- Anything already in AGENTS.md or CLAUDE.md files.
- Ephemeral task details only useful in the current conversation.

## How to Save

1. Write the memory file to the memory directory using the Write tool.
2. Update MEMORY.md in the same directory — each entry is one line under ~150 chars:
   \\\`- [Title](filename.md) — one-line hook\\\`

## How to Use

- Memory index (MEMORY.md) is loaded into your context automatically each conversation.
- Use Read to access full memory file content when needed.
- Before acting on a memory, verify it is still current (files may have changed since it was written).
- Update or remove memories that are wrong or outdated.
- Do not write duplicate memories — check for existing ones to update first.`;

export function getMemoryInstructions(): Promise<string | null> {
  return Promise.resolve(MEMORY_INSTRUCTIONS);
}
